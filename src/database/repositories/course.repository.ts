...
export class CourseRepository implements ICourseRepository {
    ...
    async create(course: ICourse): Promise<ICourseResponse> {
        try {
            ...
            const existingCourse = await this.findOneByQuery({enrolled_students:{ $in: course.enrolled_students }});
            
            if (existingCourse) {
                throw new DuplicateError("You're already enroll for this course!");
            }
    ...
    async findAll(): Promise<ICourseResponse[]> {
        try {
            ...
            throw new ApiError(`Unexpected error occurred while finding all courses!`);
        }
    }

    async updateById(id: string, course: Partial<ICourse>): Promise<ICourseResponse> {
        try {
            const existingCourse = await this.findOneByQuery({_id: new ObjectId(id), is_deleted: false})
    ...
    async deleteById(id: string): Promise<void> {
        try {
            ...
            const deletedCourse = await this.findOneByQuery({_id: new ObjectId(id) , is_deleted: true})

            if(deletedCourse){
                throw new DuplicateError("Course has been deleted!")
            }
    ...
    async searchByQuery(query: FilterQuery<CourseQuery>): Promise<ICourseResponse[]> {
        try {
            ...
            if (error instanceof NotFoundError) {
                ...
                throw new ApiError(`Unexpected error occurred while searching for Courses`);
            }
        }
    }
